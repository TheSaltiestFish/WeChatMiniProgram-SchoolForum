...
Page({
  ...
  delete: function (e) {
    ...
    console.log(info)
    db.collection('chat').doc(info._id).remove({
      success: function (res) {
  ...
  // <view class="box_bottom" bindtap="toreply" data-word="{{item._id}}" data-from="{{item.writer}}" 
    //   data-title="{{item.title}}" data-info="{{item.info}}" data-time="{{item.sendTime}}">
  toreply: function(e) {                
    wx.navigateTo({
      ...
      +'&info0='+e.currentTarget.dataset.info+'&time='+e.currentTarget.dataset.time,
    })
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
